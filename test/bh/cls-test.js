...
describe('ctx.cls()', function() {
  ...
  it('should set cls', function() {
    compile(function() {
      ...
      .apply({ block: 'button' }).should.equal('<div class="button btn"></div>');
  });
  ...
  xit('should escape cls', function() {
    compile(function() {
      ...
      .apply({ block: 'button' }).should.equal('<div class="button url=&quot;a=b&amp;c=d&quot;"></div>');
  });
  xit('should not override user cls', function() {
    compile(function() {
      block('button').cls()('btn');
    })
      .apply({ block: 'button', cls: 'user-btn' }).should.equal('<div class="button user-btn"></div>');
  });
  ...
  xit('should override later declarations with force flag', function() {
    compile(function() {
      ...
      .apply({ block: 'button' }).should.equal('<div class="button control"></div>');
  });
  xit('should override user declarations with force flag', function() {
    compile(function() {
      block('button').cls().force()('btn');
    })
      .apply({ block: 'button', cls: 'user-btn' }).should.equal('<div class="button btn"></div>');
  });
});
