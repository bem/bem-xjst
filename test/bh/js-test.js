...
describe('js()(×)', function() {
  ...
  it('should set js', function() {
    compile(function() {
      ...
      .apply({ block: 'button' }).should.equal('<div class="button i-bem" data-bem=\'{"button":{}}\'></div>');
  });
  ...
  it('should not override user declarations', function() {
    compile(function() {
      ...
      .apply({ block: 'button', js: false }).should.equal('<div class="button"></div>');
  });
  it('should extend user js', function() {
    compile(function() {
      ...
      .apply({ block: 'button', js: { x: 1 } })
        .should.equal('<div class="button i-bem" data-bem=\'{"button":{"x":1,"a":2}}\'></div>');
  });
  it('should extend user js, when it equal true', function() {
    compile(function() {
      block('button').js()({ a: 1 });
    })
      .apply({ block: 'button', js: true }).should.equal('<div class="button i-bem" data-bem=\'{"button":{"a":1}}\'></div>');
  });
  ...
  xit('should override user declarations with force flag', function() {
    compile(function() {
      ...
      .apply({ block: 'button', js: { a: 1 } }).should.equal('<div class="button"></div>');
  });
});
