...
function BEMXJST(options) {
  ...
  this.options.locals = this.options.Tree.methods.concat(
    'local', 'applyCtx', 'applyNext', 'apply'
  );
...
BEMXJST.prototype.compile = function compile(code) {
  ...
  var tree = new this.options.Tree({
    refs: {
...
BEMXJST.prototype.recompileInput = function recompileInput(code) {
  ...
  if (typeof code === 'function' && code.length === this.options.locals.length)
    return code;
  ...
  out = new Function(this.options.locals.join(', '), out);
