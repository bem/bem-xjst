...
BEMXJST.prototype.warm = function(json) {
  if (!json || !json.block) return;

  var key = this.classBuilder.build(json.block, json.elem);
  ...
  Object.keys(json).forEach(function(key) {
    ...
    if (key === 'attrs') {
      ...
      Object.keys(attrs).forEach(function(attrKey) {
        ctx.attrs[attrKey] = `{{attrs.${attrKey}}}`;
      });
  ...
  var template = this.run(ctx);

  this.warmTemplates[key] = function(json) {
    return Object.keys(ctx).reduce(function(html, key) {
      if (entityKeys[key]) return html;

      if (key === 'attrs') {
        return Object.keys(ctx.attrs).reduce(function(html, attrKey) {
          var replacer = new RegExp('{{attrs.' + attrKey + '}}', 'g');

          return html.replace(replacer, json.attrs[attrKey]);
        }, html);
      }

      var replacer = new RegExp('{{' + key + '}}', 'g');

      return html.replace(replacer, json[key]);
    }, template);
  };
}
...
BEMXJST.prototype.runOne = function(json) {
  var key = this.classBuilder.build(json.block, json.elem);
  //console.log('KEY BLYAT #####', key);
  //console.log('WARM BLYAT #####', this.warmTemplates[key]);

  if (this.warmTemplates[key]) return this.warmTemplates[key](json);
  var context = this.context;
