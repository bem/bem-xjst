...
function getDeps(requires) {
  ...
  if (deps.ymLibs.length) {
    deps.ymVarsString = deps.ymLibs.reduce(function(vars, item) {
      return vars + (vars ? ',' : '') + item + ':' + item;
    }, '');

    deps.ymEngineLibs = 'engine.libs = {' + deps.ymLibs.map(function(item) {
      return '"' + item + '":' + item;
    }).join(',') + '};';
  }
  ...
  file
    ...
    .writeContent([
      ...
      'modules.define("' + exportName + '",' +
        ...
        'function(provide' +
        (deps.ymLibs.length ? ',' + deps.ymLibs.join(',') : '') + ') { ' +
