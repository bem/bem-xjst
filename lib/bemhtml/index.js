...
BEMHTML.prototype.renderClose = function renderClose(prefix,
  ...
  if (utils.isShortTag(tag)) {
    ...
    var optionalClosingTags = [
      // html4
      'html', 'head', 'body', 'p', 'ul', 'ol', 'li', 'dt', 'dd',
      'colgroup', 'thead', 'tbody', 'tfoot', 'tr', 'th', 'td', 'option',
      // html5
      /* dl — Neither tag is omissible */ 'rb', 'rt', 'rtc', 'rp', 'optgroup'
    ];

    if (optionalClosingTags.indexOf(tag) === -1)
      out += '</' + tag + '>';
  }
