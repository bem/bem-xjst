...
function BEMHTML(options) {
  ...
  this._entitiesProcessed = typeof options.entitiesProcessed !== 'undefined' && {};
}
...
BEMHTML.prototype.render = function(context, entity, tag, js, bem, cls, mix,
  ...
  if (isBEM) {
    if (mix)
      m = this.renderMix(entity, mix, jsParams, addJSInitClass);

    classValue += entity.jsClass;

    if (this._entitiesProcessed)
      this._entitiesProcessed[entity.jsClass] = true;
  ...
  if (cls) {
    var clsName = typeof cls === 'string' ? utils.attrEscape(cls).trim() : cls;

    classValue += isBEM ? ' ' + clsName : clsName;

    if (this._entitiesProcessed)
      this._entitiesProcessed[clsName] = true;
  }
...
BEMHTML.prototype.renderMix = function(entity, mix, jsParams, addJSInitClass) {
  ...
  for (var i = 0; i < mix.length; i++) {
    ...
    if (hasItem) {
      var className = classBuilder.build(block, classElem)

      out += ' ' + className;

      if (this._entitiesProcessed)
        this._entitiesProcessed[className] = true;
    }
