...
BEMHTML.prototype._mapClass = function _mapClass(classname, separator) {
  if (!this._classMap) return classname;

  return separator ?
    classname.split(separator).map(this._mapClass, this).join(separator) :
...
BEMHTML.prototype.render = function render(context,
  ...
  if (isBEM) {
    ...
    out += this._mapClass(entity.jsClass);
...
BEMHTML.prototype.renderMix = function renderMix(entity,
  ...
  for (var i = 0; i < mix.length; i++) {
    ...
    if (hasItem)
      out += ' ' + this._mapClass(classBuilder.build(block, classElem));
