...
Compiler.prototype.pretranslate = function pretranslate(ast) {
  ...
  ast = estraverse.replace(ast, {
    ...
    leave: function(ast, parent) {
      ...
      var extendedModeDetected = ast.extendedModeDetected;
      
      // 1. mark all match calls
      ast = self.markMatches(ast);

      // Propagate `replace' flag if detected in markMatches or set in children
      if (ast.extendedModeDetected || extendedModeDetected) {
        extendedModeDetected = ast.extendedModeDetected || extendedModeDetected; 
        parent.extendedModeDetected = extendedModeDetected;
      }
      ...
      if (extendedModeDetected) ast.extendedModeDetected = extendedModeDetected;
      return ast;
    }
  });
...
Compiler.prototype.markMatches = function markMatches(ast) {
  ...
  if (ast.type !== 'CallExpression') {
    if (ast.type === 'Identifier' &&
      ...
      // Desugar`replace' mode predicate and set the flag 
      ast.extendedModeDetected = ast.name;
      ast.name = 'def';
    }
    return ast;
  }
...
Compiler.prototype.flatten = function flatten(expr) {
  var extendedModeDetected = expr.extendedModeDetected;
  ...
  return dive(expr).map(function(t) {
    return {
      ...
      arguments: [ this.replaceBody(t.body, extendedModeDetected) ]
    };
  }, this);
};

Compiler.prototype.replaceBody = function replaceBody(body, extendedModeDetected) {
  var self = this;

  // Desugar`replace' mode body
  if (extendedModeDetected) {
    body.body = estraverse.replace(body.body, {
      ...
      leave: function (ast) {
        if (ast.type === 'ReturnStatement') {
          var applyCtxArgNode, applyCtxNode;

          if (extendedModeDetected === 'replace') {
            applyCtxArgNode = ast.argument;
          } else if (extendedModeDetected === 'extend') {
            applyCtxArgNode = {
              type: 'CallExpression',
              callee:
              { type: 'MemberExpression',
