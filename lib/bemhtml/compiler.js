...
Compiler.prototype.pretranslate = function pretranslate(ast) {
  ...
  ast = estraverse.replace(ast, {
    ...
    leave: function(ast, parent) {
      ...
      var replaceModeDetected = ast.replaceModeDetected;
      
      // 1. mark all match calls
      ast = self.markMatches(ast);

      // Propagate `replace' flag if detected in markMatches or set in children
      if (ast.replaceModeDetected || replaceModeDetected) {
        replaceModeDetected = parent.replaceModeDetected = true; 
      }
      ...
      if (replaceModeDetected) ast.replaceModeDetected = replaceModeDetected;
      return ast;
    }
  });
...
Compiler.prototype.flatten = function flatten(expr) {
  var replaceModeDetected = expr.replaceModeDetected;
  ...
  return dive(expr).map(function(t) {
    return {
      ...
      arguments: [ this.replaceBody(t.body, replaceModeDetected) ]
    };
  }, this);
};

Compiler.prototype.replaceBody = function replaceBody(body, replaceModeDetected) {
  var self = this;

  // Desugar`replace' mode body
  if (replaceModeDetected) {
