...
VIDOM.prototype.runMany = function runMany(arr) {
  ...
  if (out.filter(function(o) { return typeof o === 'string'; }).length === out.length) {
    out = out.join('');
  }
...
BEMXJST.prototype.runSimple = function runSimple(context) {
  this.context._listLength--;
  var res = '';
  var isChild = !!this.context.block || !!(this.context.ctx && this.context.ctx.tag);
...
VIDOM.prototype.render = function render(
        ...
        if (mixEntity.block) { // add mods
          props.className = props.className.concat(
            this.buildClasses(
              ...
              mixEntity.elem,
              mixEntity.elem
                ? (mixEntity.elemMods || mixEntity.mods)
                : mixEntity.mods
              )
          );
        }
  ...
  if (children) {
    var isOneElement = !Array.isArray(children)
      || children.length > 1 && typeof children[1] === 'object' && !Array.isArray(children[1]);
    if(isOneElement) {
      viArguments.push(children);
    } else {
...
VIDOM.prototype.buildClasses = function buildClasses(block, elem, mods) {
  ...
  for (var modName in mods) {
    ...
    naming.validate(entity) || classes.push(naming.stringify(entity));
  }
