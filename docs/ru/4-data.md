...
`{String}` HTML-класс или классы (через пробел), не принадлежащие к БЭМ-предметной области (блок-элемент-модификатор). Например, применение [микроформатов](http://microformats.org/) или семантической разметки из [schema.org](https://schema.org/).
